...
#' dataframe. 
#' 
#' Can be used to remove cases where the window was resized or to remove participants who used a touch device.
#'
#' @param data A dataframe with the mouse movement data. 
#' @param part_id The participant ID.
#' @param sess_id The session or worker ID, identifies the seperated session, one participant has multiple IDs. 
...
#' @return A filtered dataframe with removed cases. 
#' @examples
#' rm_cases(data, factor_rm = "resize", criteria = 1)
#' rm_cases(data, column_rm = "userAgent_is_touch_capable", factor_rm = TRUE)
#' @export



rm_cases <- function(data, part_id = "mouseid", sess_id = "workerId", column_rm = "type", factor_rm, criteria) {
