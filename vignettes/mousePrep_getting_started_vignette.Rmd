...
| Question or screen ID | `question_id`, `screen_id` | Survey item, screen, or task identifier |
| Time variable | `timestamps`, `timeStamp`, `time` | Time of each recorded event |
| Coordinates | `xpos`, `ypos`, `ClientX`, `ClientY` | Mouse position |
| Screen dimensions | `inner_width`, `inner_height`, `scroll_width`, `scroll_height` | Browser and page dimensions |
...
Use `rm_cases()` to remove invalid participants, screens, or empty trajectories.
...
If certain type of trajectories have to be subsetted, use `sl_cases`.
...
`screen_dims_calculate()` computes screen width and height from available inner and scroll dimensions.
...
`mousePrep` helps organize the messy but important preprocessing steps in mouse-tracking research. The package is especially useful when raw data has inconsistent column names, mixed event types, device-related noise, timing issues, repeated screen visits, or needs to be prepared for analysis with `mousetrap`.
...
2. clean,
3. engineer features,
4. flag and remove outliers,
5. resolve repeated trajectories,
6. process with `mousetrap`, and
7. export long or wide datasets.
